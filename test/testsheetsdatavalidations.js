...
export const testSheetsDataValidations = (pack) => {
  ...
  const scritty = (t, sb, range, prop, method, values = []) => {
    if (noisy)console.log ("scritty",prop,method,values,range)
    const cr = sb.getRange(range)
    ...
    // TODO - how to handle values in list?
    ...
    if (is.emptyArray(values)) {
      ...
      if (noisy)console.log ('built dv - no values for', cr.getA1Notation())
      cr.setDataValidation(built)
      const cb = cr.getDataValidation()
      t.is(cb.getCriteriaType().toString(), prop)
      t.deepEqual(cb.getCriteriaValues(), [])

    } else if (!is.array(values[0])) {
      const built = method ? nv[method](...values).build() : nv.withCriteria(crit, values).build()
      cr.setDataValidation(built)
      if (noisy)console.log ('built dv - single value for', cr.getA1Notation())
      const cbs = cr.getDataValidations()
      if (noisy)console.log ('received rows', cbs.length)
  ...
  unit.section("setting data validations", t => {
    ...
    // TODO this doesnt work for scritty
    scritty(t, sb, "c15:d15", "DATE_BETWEEN", "requireDateBetween", [[[[da, db]],[[da,db]]],[[[da,da]],[[db,db]]]])
    ...
    scritty(t, sb, "a19:b19", "VALUE_IN_RANGE", "requireValueInRange",["$a$1:$b$3"])

    // TODO figure out how to handle this for scritty because arg is itself an array
    scritty(t, sb, "c19:e19", "VALUE_IN_LIST", "requireValueInList",[["foo","bar","foobar"]])
