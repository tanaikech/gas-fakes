...
/**
 * Validates arguments for the _elementInserter function.
 * @private
 */
const _validateInserterArgs = (elementOrText, childIndex, options) => {
  const {
    elementType,
 ...
 * @private
 */
const _calculateInsertionPointsAndInitialRequests = (self, childIndex, isAppend, shadow) => {
  const structure = shadow.structure;
  const item = structure.elementMap.get(self.__name);
 ...
 * @private
 */
const _findAndReturnNewElement = (self, shadow, insertIndex, newElementStartIndex, isAppend, options) => {
...
const _elementInserter = (self, elementOrText, childIndex, options) => {
  ...
  const { isAppend, isDetached } = _validateInserterArgs(elementOrText, childIndex, options);
  ...
  const { insertIndex, newElementStartIndex, requests } = _calculateInsertionPointsAndInitialRequests(
  ...
  return _findAndReturnNewElement(self, shadow, insertIndex, newElementStartIndex, isAppend, options);
};
