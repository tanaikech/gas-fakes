# gas-fakes v2.5.6 Release Notes

## Overview
Version 2.5.6 introduces support for the Advanced Slides `Presentations.Pages.getThumbnail` method, alongside proxy capability enhancements for underlying nested properties in `sxslides.js`. 

## Key Features & Additions

### 1. Advanced Slides Service Enhancements
- **Thumbnail Support**: Added `FakeAdvSlidesPresentationsPages` to implement `Slides.Presentations.Pages.getThumbnail(presentationId, pageObjectId, options)`.
- **Proxy Enhancements**: Updated `sxslides.js` to correctly support nested properties via the `subProp` argument, enabling deep API client calls such as `apiClient.presentations.pages.getThumbnail`.
- **Expanded Testing**: Added thumbnail tests to `test/testslidesadv.js` that mock presentations, fetch different sizes (`SMALL`, `MEDIUM`, `LARGE`), and validate sizes.

### 2. Version Updates
- Updated core `package.json` to v2.5.6.
- Updated `test/package.json` to v2.5.6.
