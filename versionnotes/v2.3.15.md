...
### 2. EmbeddedChartBuilder Visual Methods
Dozens of previously empty "stub" methods on the `EmbeddedChartBuilder` have been activated to translate directly to the Google Sheets REST API v4 JSON payload.
- **Implemented Methods**: `setColors()`, `setXAxisTitle()`, `setYAxisTitle()`, `setRange()`, `setXAxisRange()`, `setYAxisRange()`, `setStacked()`, `setBackgroundColor()`, `setLegendPosition()`, and `set3D()`.
