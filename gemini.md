...
- If the `activate_skill` tool is available, always activate the `gas-fakes-dev` skill at the beginning of every session or task.
- If the `activate_skill` tool is available, always activate the `gf_agent` skill at the beginning of every session or task.