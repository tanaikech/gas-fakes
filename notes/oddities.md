...
### Chart Generation API Oddities

When using the `SpreadsheetApp.newChart()` builders, several undocumented differences exist between the in-memory builder states on Live Apps Script versus the strictly enforced JSON requirements of the Google Sheets REST API.

#### Eager Method Signature Validation

Live Apps Script performs highly rigid backend validation on enum types *before* evaluating method bodies. Passing string literals instead of explicit Enums (e.g., passing `"SHOW_ALL"` instead of `Charts.ChartHiddenDimensionStrategy.SHOW_ALL`) works in plain JavaScript (and `gas-fakes`), but crashes Live Apps Script instantly with `The parameters (String) don't match the method signature`.

#### Formatting Method Availability on Builders

The generic `EmbeddedChartBuilder` returned by `sheet.newChart()` does not officially support visual formatting methods like `.setTitle()`, `.setBackgroundColor()`, or `.setLegendPosition()`. In Live Apps Script, these methods are only available on the specialized sub-builders (e.g., `EmbeddedPieChartBuilder`). Attempting to chain `.setTitle()` directly onto the root builder throws a `TypeError: is not a function`.

However, `setHiddenDimensionStrategy` **is** formally defined on the root builder, yet calling it prior to assigning a specific chart type, or calling it on a `PieChartBuilder`, will trigger an opaque `Unexpected error` backend crash.

#### Dynamic `toString()` Signatures

The `.toString()` output of a chart builder in Live Apps Script changes dynamically depending on its internal state. A fresh root builder outputs `com.google.apps.maestro.server...`, but the instant it is coerced via `.asPieChart()`, its `.toString()` morphs cleanly into `"EmbeddedPieChartBuilder"`.

#### Sheets REST API vs. Apps Script Builder Objects

The Google Sheets REST API requires highly specialized JSON `spec` layouts depending on the chart type, which do not align 1-to-1 with the `SpreadsheetApp` builder API:

1.  **Legend Positions:** Apps Script utilizes enums like `BOTTOM`, while the REST API strictly demands `BOTTOM_LEGEND`.
2.  **Pie Charts and Histograms:** These types cannot exist within a standard `basicChart` spec wrapper. The REST API requires `gas-fakes` to pivot these configurations into top-level `pieChart` and `histogramChart` spec structures, removing the `basicChart` shell entirely.
3.  **Combo Charts:** While a COMBO chart utilizes the `basicChart` wrapper, the REST API refuses to render it unless every single underlying data series contains an explicit `type` definition (e.g., `LINE` or `COLUMN`). In Apps Script, the default visual fallback for an unassigned COMBO series is a `COLUMN` chart, requiring `gas-fakes` to proactively inject that fallback type into the API payload.
